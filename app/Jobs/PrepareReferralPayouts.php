...
class PrepareReferralPayouts implements ShouldQueue
{
    use Queueable;
    ...
    {
        Log::info('PrepareReferralPayouts: Starting to process payout items');
        ...
        DB::transaction(function () {
            ...
            if ($items->isEmpty()) {
                Log::info('PrepareReferralPayouts: No items to process');

                return;
            }

            Log::info('PrepareReferralPayouts: Found '.$items->count().' items to process');
            ...
            foreach ($grouped as $enrollmentId => $enrollmentItems) {
                ...
                Log::info("PrepareReferralPayouts: Processing enrollment {$enrollmentId} - Amount: {$totalAmount}, Count: {$referralCount}");
                ...
                Log::info("PrepareReferralPayouts: Created payout #{$payout->id} for enrollment {$enrollmentId}");
            }

            Log::info('PrepareReferralPayouts: Successfully processed all items');
        });
    }
}
