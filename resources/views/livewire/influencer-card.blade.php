    ...
    <div class="relative px-4 pb-4">
        ...
        <div class="absolute -top-8 left-4">
            <div class="w-16 h-16 rounded-full border-4 border-white dark:border-gray-800 shadow-md overflow-hidden bg-white">
                <img src="{{ $profileImageUrl }}" alt="{{ $influencer->user?->name }}" class="w-full h-full object-cover">
            </div>
            @if($isVerified)
        ...
        <div class="pt-10">
            <div class="flex items-start justify-between">
                <div class="min-w-0 flex-1">
                    ...
                    @if($influencer->postalCodeInfo)
                        <p class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-0.5">
                            <flux:icon.map-pin class="w-3 h-3" />
                            {{ $influencer->postalCodeInfo->place_name }}, {{ $influencer->postalCodeInfo->admin_code1 }}
                            @if(isset($influencer->distance) && $influencer->distance !== null)
                                <span class="text-gray-400">·</span>
                                <span>{{ number_format($influencer->distance, 1) }} mi</span>
                            @endif
                        </p>
                    @endif
                </div>
        ...
        <div class="flex gap-2 pt-2">
            ...
            <flux:button
                ...
                size="sm"
                class="flex-1"
                wire:click="$dispatch('open-invite-modal', { influencerId: {{ $influencer->user?->id }}, influencerName: {{ json_encode($influencer->user?->name) }} })"
            >
                Invite
            </flux:button>
            @endif
